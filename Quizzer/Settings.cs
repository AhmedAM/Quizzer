...
namespace Quizzer
{
    public partial class Settings : Form
    {
        ...
        {
            dt =connect.CustomSelect("Category_Name", "Category_Main", "Order by Category_Order");
      ...
      // Check box Options Start 
        ...
        private void CategoryListUpdater()
        {
            dt_category = connect.CustomSelect("Category_ID , Category_Name , Category_Order ", "Category_Main", " Order by Category_Order");
            list_rounds.DataSource = dt_category;
            list_rounds.DisplayMember = "Category_Name";
        }


        private void bt_addcategory_Click(object sender, EventArgs e)
        {
            dt= connect.CustomSelect("max(Category_Order)","Category_Main","");
            int max = Convert.ToInt32(dt.Rows[0].ItemArray[0].ToString());
            max += 1;
            connect.CustomInsertion("Category_Main", "Category_Name,Category_Order", "'" + tb_category.Text + "' , " + max + "");
            CategoryListUpdater();
        }

        private void bt_deletec_Click(object sender, EventArgs e)
        {
            try
            {
                DataRow[] R;
                R = dt_category.Select("Category_Name = '" + list_rounds.Text + "'");
                connect.CustomNonQuery("Delete from Category_Main where Category_ID = " + R[0].ItemArray[0].ToString() + "");
                CategoryListUpdater();
                bt_deletec.Visible = false;
            }
            catch
            { 
        ...
        private void list_rounds_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (list_rounds.Text != "" || list_rounds.Text != null)
                    bt_deletec.Visible = true;
                else
                {
                    bt_deletec.Visible = false;
                }
        ...
        {
            DataRow[] R;
            int indexselected = list_rounds.SelectedIndex;// Selected Items Index 
            string select = list_rounds.Text;
            R = dt_category.Select("Category_Name = '" +select+"'");
            int order = Convert.ToInt32(R[0].ItemArray[2].ToString()); // Selected Items current Order
            int id = Convert.ToInt32(R[0].ItemArray[0].ToString()); // select items ID 
            order -= 1;
            connect.CustomNonQuery("Update Category_Main Set Category_Order = " + order + " where Category_ID = " + id + "");

            order += 1;
            indexselected -= 1;
            list_rounds.SelectedIndex = indexselected;
            select = list_rounds.Text;
            R = dt_category.Select("Category_Name = '" + select + "'");
             //order = Convert.ToInt32(R[0].ItemArray[2].ToString());  
             id = Convert.ToInt32(R[0].ItemArray[0].ToString());
             connect.CustomNonQuery("Update Category_Main Set Category_Order = " + order + " where Category_ID = " + id + "");
